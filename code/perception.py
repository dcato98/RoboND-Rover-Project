...
# Define a function to identify rock pixels
...
def perception_step(Rover):
    ...
    # Color threshold - identify navigable terrain and obstacles
    threshed = color_thresh(warped)
    obs_map = np.absolute(np.float32(threshed) - 1) * mask
    
    # Update Rover.vision_image (displayed on left side of screen)
    ...
    scale = 2*dst_size
    xworld, yworld = pix_to_world(xpix, ypix, Rover.pos[0], Rover.pos[1], 
                                  Rover.yaw, world_size, scale)
    obs_xworld, obs_yworld = pix_to_world(obs_xpix, obs_ypix, Rover.pos[0], Rover.pos[1], 
                                          Rover.yaw, world_size, scale)
    
    # Update Rover.worldmap (displayed on right side of screen)
    ...
    # Convert rover-centric pixel coordinates to polar space
    ...
    if rock_map.any():
        ...
        rock_xworld, rock_yworld = pix_to_world(rock_xpix, rock_ypix, Rover.pos[0], Rover.pos[1], 
                                                Rover.yaw, world_size, scale)
        rock_dists, rock_angles = to_polar_coords(rock_xpix, rock_ypix)
        rock_idx = np.argmin(rock_dists)
        rock_xcen = rock_xworld[rock_idx]
        rock_ycen = rock_yworld[rock_idx]
